...
const TableCell = ({ children, className, onClick, padding = true, cellStyle = 'white', size = "lg", align = "left" }: TableCellProps) => {
    ...
    return (
        <td onClick={onClick} className={cn('py-[14px] text-grey-400 text-sm', sizeStyle, alignStyle, padding ? "px-6" : "", cellStyle == 'white' ? "bg-white " : "bg-grey-50", className)}>{children}</td>
    )
}
