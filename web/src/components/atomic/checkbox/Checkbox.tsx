        ...
        <>
            <div className='flex flex-row items-center justify-start gap-2 '>
                <input name={name} required={required} type="checkbox" id={id} onChange={onChange} checked={checked}
                    className={cn("  text-primary-500 hover:cursor-pointer duration-150 ease-in-out  shadow-sm  focus:ring-0 focus:ring-offset-0  !outline-none rounded-[4px] border-[1px] border-grey-300", className)} />
                <label htmlFor={id} className='text-sm leading-5 hover:cursor-pointer'>{label}</label>
            </div>
            {error &&
